L.Control.standorteControl = L.Control.extend({
	...
	initialize: function(options) {
		...
		this._standorteIcon = L.icon({
			iconUrl: 'resources/images/pin1.png'
			, iconSize: [37, 65]
			, iconAnchor: [18, 63]
			, popupAnchor: [0, -63]
		});
	...
	onAdd: function(map) {
		this._initLayout();
		this._map.on('updatestandortecontrol', this._update_standortecontrol, this);
		return this._container;
	},

	_initLayout: function() {
		var classname = 'standorte';
		...
		input.type = 'checkbox';
		input.id = 'idstandorteControlCenter';
		...
		input1.id = 'idstandorteControlR1';
		...
		select1.id = 'idstandorteControlS1';
		...
		input2.id = 'idstandorteControlR2';
		...
		select2.id = 'idstandorteControlS2';
	...
	_markPosition: function() {
		var input = document.getElementById('idstandorteControlCenter')
		...
		if (input.checked) {
			...
			this._marker = L.marker(center, {draggable: true, icon: this._standorteIcon});
	...
	_onInputClickPosition: function() {
		this._markPosition();
		//this._map.fire('standortecontrolchanged');
		this._map.fire('_update_standortecontrol');
	},
	...
	_drawRadius1: function() {
		...
		var input = document.getElementById('idstandorteControlR1')
		if (input.checked) {
			this.options.c1 = 1;
			var center;
			if (this.options.mlat && this.options.mlon) {
			...
			this._circle1 = L.circle(center, this.options.r1*1000, {clickable: true,color : "red"}).addTo(this._map);
		}
	...
	_onSelectRadius1: function() {
		var select = document.getElementById('idstandorteControlS1')
	...
	_drawRadius2: function() {
		...
		// draw circle if needed
		var input = document.getElementById('idstandorteControlR2')
		if (input.checked) {
			this.options.c2 = 1;
			var center;
			if (this.options.mlat && this.options.mlon) {
	...
	_onSelectRadius2: function() {
		var select = document.getElementById('idstandorteControlS2')
	...
	_update_standortecontrol: function(e) {
		if (e.params.r1) {
			for (var i=0; i<this._r1_list.length; i++) {
				if (this._r1_list[i] == e.params.r1) {
					this.options.r1 = e.params.r1;
					var elem1 = document.getElementById('idstandorteControlS1');
					...
					window.setTimeout(function() {
						document.getElementById('idstandorteControlS1').selectedIndex = _s1idx;
					}, 200)
				}
			}
		}
		if (e.params.r2) {
			for (var i=0; i<this._r2_list.length; i++) {
				if (this._r2_list[i] == e.params.r2) {
					...
					var elem2 = document.getElementById('idstandorteControlS2');
					...
					window.setTimeout(function() {
						document.getElementById('idstandorteControlS2').selectedIndex = _s2idx;
					}, 210)
				}
			}
		}
		...
		if (e.params.c1 && this.options.mlat && this.options.mlon) {
			this.options.c1 = 1;
			document.getElementById('idstandorteControlR1').checked = true;
			this._drawRadius1();
		}
		if (e.params.c2 && this.options.mlat && this.options.mlon) {
			this.options.c2 = 1;
			document.getElementById('idstandorteControlR2').checked = true;
			this._drawRadius2();
		}
		if (e.params.m && this.options.mlat && this.options.mlon) {
			this.options.m = 1;
			document.getElementById('idstandorteControlCenter').checked = true;
			this._markPosition();
		}
	},
	...
	initMarker: function(askGeolocation) {
		...
		document.getElementById('idstandorteControlCenter').checked = true;
		this._markPosition();
		 this._marker.bindPopup("Standort").openPopup();
		
	}

});

L.control.standorteControl = function(options) {
	return new L.Control.standorteControl(options);
}
